...
const IconMap: Record<string, React.ReactNode> = {
  'thunder': <Code2 className="w-5 h-5" />,
  'syncro': <Cpu className="w-5 h-5" />,
  'mitr-core': <Database className="w-5 h-5" />,
};
              ...
              <div className="p-8">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center text-primary shadow-sm group-hover:bg-primary group-hover:text-white transition-colors">
                    {IconMap[project.id] || <Code2 className="w-5 h-5" />}
                  </div>
                  <div>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {project.tags.map(tag => (
                    <span key={tag} className="text-[10px] font-bold px-3 py-1 rounded-full bg-white border border-slate-100 text-slate-400 uppercase tracking-widest">
                      {tag}
                    </span>
                  ))}
                </div>
