...
export function ServerModule({ position, serverId = 0 }: { position: [number, number, number], serverId?: number }) {
  ...
  return (
    <group position={position}>
      {/* 🛡️ LAYER 1: OUTER REINFORCED SHELL (Light Gray Metal) */}
      ...
      {/* 🛡️ EMISSIVE RIM HIGHLIGHT (Edge Glow) */}
      <mesh position={[0, SERVER_HEIGHT, 0]}>
         <boxGeometry args={[SERVER_WIDTH + 0.2, 0.1, SERVER_DEPTH + 0.2]} />
...
function StatusLED({ bladeIndex, serverId }: { bladeIndex: number, serverId: number }) {
  ...
  const config = useMemo(() => {
    ...
    return {
      ...
      color: seed % 7 === 0 ? "#ef4444" : seed % 4 === 0 ? "#f59e0b" : "#2563eb",
